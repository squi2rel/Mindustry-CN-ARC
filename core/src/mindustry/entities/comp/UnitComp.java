...
abstract class UnitComp implements Healthc, Physicsc, Hitboxc, Statusc, Teamc, Itemsc, Rotc, Unitc, Weaponsc, Drawc, Boundedc, Syncc, Shieldc, Displayable, Ranged, Minerc, Builderc, Senseable, Settable{
    ...
    public void rotateMove(Vec2 vec){
        ...
        if(!vec.isZero()){
            rotation = Angles.moveToward(rotation, vec.angle(), Hack.immediatelyTurn ? 360f : type.rotateSpeed * Time.delta * speedMultiplier);
        }
    }
    ...
    public boolean inFogTo(Team viewer){
        if(Hack.noFog || this.team == viewer || !state.rules.fog) return false;
    ...
    public void setProp(LAccess prop, double value){
        switch(prop){
            ...
            case x -> {
                if (controller instanceof Player pl && pl == player && Hack.ignoreProcessor) return;
